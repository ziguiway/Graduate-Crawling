...
# 按单元格索引精确翻译（只翻译 markdown 单元格）
# 先收集所有 markdown 单元格的索引
md_indices = [i for i, c in enumerate(nb['cells']) if c['cell_type'] == 'markdown']
print(f"Markdown 单元格索引: {md_indices}")
print(f"共 {len(md_indices)} 个 markdown 单元格")

# 按顺序定义每个 markdown 单元格的翻译
translations_by_index = {
    ...
    10: [  # Create numpy array of labels
        "* 创建积极标签和消极标签的 numpy 数组。"
    ],
    13: [  # Create frequency dictionary
    ...
    18: [  # Expected output - process tweet
        "#### 预期输出\n",
        "```\n",
    ...
    19: [  # Part 1: Logistic regression / Sigmoid
        "# 第 1 部分：逻辑回归\n",
        "\n",
        "\n",
        "### 第 1.1 节：Sigmoid 函数\n",
    ...
    24: [  # Logistic regression: regression and sigmoid
        "### 逻辑回归：回归 + sigmoid\n",
        "\n",
    ...
    27: [  # Likewise if model predicts close to 0
    ...
    30: [  # Instructions gradient descent
        "## 说明：实现梯度下降函数\n",
    ...
    34: [  # Expected output - gradient descent
        "#### 预期输出\n",
        "```\n",
    ...
    35: [  # Part 2: Extracting the features
        "## 第 2 部分：提取特征\n",
        "\n",
    ...
    39: [  # Expected output - extract_features test1
        "#### 预期输出\n",
        "```\n",
        "[[1.00e+00 3.02e+03 6.10e+01]]\n",
        "```"
    ],
    41: [  # Expected output - extract_features test2
        "#### 预期输出\n",
        "```\n",
        "[[1. 0. 0.]]\n",
        "```"
    ],
    42: [  # Part 3: Training Your Model
        "## 第 3 部分：训练你的模型\n",
        "\n",
        "训练模型：\n",
        "* 将所有训练样本的特征堆叠成矩阵 `X`。\n",
    ...
    45: [  # Part 4: Test your logistic regression
        "# 第 4 部分：测试你的逻辑回归\n",
        "\n",
    ...
    48: [  # Expected Output - predict_tweet
        "**预期输出**：\n",
        "```\n",
        "I am happy -> 0.518580\n",
        "I am bad -> 0.494339\n",
    ...
    50: [  # Check performance using the test set
        "## 使用测试集检查性能\n",
    ...
    51: [  # Hints test_logistic_regression
        "<details>    \n",
        "<summary>\n",
        "    <font size=\"3\" color=\"darkgreen\"><b>提示</b></font>\n",
        "</summary>\n",
    ...
    54: [  # Expected Output - accuracy
        "#### 预期输出：\n",
        "```0.9950```  \n",
        "相当不错！"
    ],
    55: [  # Part 5: Error Analysis
        "# 第 5 部分：错误分析\n",
        "\n",
        "在这一部分，你将看到一些模型分类错误的推文。你认为为什么会发生这些分类错误？具体来说，你的模型会对哪种类型的推文分类错误？"
    ],
    57: [  # Later in specialization
        "在本专项课程的后续内容中，我们将学习如何使用深度学习来提高预测性能。"
    ],
    58: [  # Part 6: Predict with your own tweet
        "# 第 6 部分：用你自己的推文进行预测"
    ],
}

# 应用翻译
for idx, translated in translations_by_index.items():
    if idx < len(nb['cells']) and nb['cells'][idx]['cell_type'] == 'markdown':
        nb['cells'][idx]['source'] = translated
    else:
        print(f"警告：单元格 {idx} 不是 markdown 或超出范围")

# 检查是否有未翻译的 markdown 单元格
for i, c in enumerate(nb['cells']):
    if c['cell_type'] == 'markdown' and i not in translations_by_index:
        print(f"未翻译的 markdown 单元格 [{i}]: {''.join(c['source'])[:80]}")
