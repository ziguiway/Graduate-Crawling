...
短视频虚假信息检测已经不是空白方向，当前方法正从简单的多模态融合，逐渐发展到因果与反事实建模、评论和社会上下文利用，以及 VLM/LLM 驱动的可解释推理。

我当前的想法不是从头训练视频模型，而是利用成熟工具提取 ASR 文本、OCR 字幕、关键帧描述和评论等信息，再交给 AI 模型进行综合判断。这个思路是否能成为研究工作，关键不在于“能不能调用大模型”，而在于能否证明它比现有方法更低成本、更可解释、更容易跨平台泛化，或者能够引入可核验的外部证据。后续调研应重点检查现有论文是否已经解决这些问题，以及公开数据和代码是否足以支持实验验证。
