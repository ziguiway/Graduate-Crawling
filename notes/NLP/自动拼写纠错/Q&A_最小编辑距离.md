...
你的疑问是成立的：如果已经用 `edit_one_letter()` 和 `edit_two_letters()` 生成了候选词，那么再对这些候选调用 `min_edit_distance()`，并不能带来新的纠错结果。

```text
edit_one_letter()  → 已经得到距离为 1 的候选
edit_two_letters() → 已经得到距离为 2 的候选
min_edit_distance() → 只能重新测量这些候选的距离
```

所以在**这个作业的自动纠错实现里**，最小编辑距离不是必需的。它没有负责生成候选，也没有参与 `get_corrections()` 的最终排序；它主要是后面单独安排的动态规划练习，也可以用来验证前面生成的候选是否真的相差 1 步或 2 步。

以后如果候选词不是通过这几个编辑函数生成的，例如候选来自整个词典或其他模型，那么最小编辑距离才可以用来给候选打分、筛掉距离太远的词。但在当前 notebook 里，先把它理解为一个独立的“字符串距离计算练习”就够了，不必强行把它和自动纠错主流程连起来。
