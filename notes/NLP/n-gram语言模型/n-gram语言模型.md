...
$$
P(W)=\prod_{t=1}^{N}P(w_t\mid w_{<t})
$$

但它不适合直接比较不同长度的句子。因为一句话的总概率是许多个小于 1 的数相乘：句子越长，即使模型每一步预测得一样好，最终乘积也会天然更小。

```text
短句：0.5 × 0.5 = 0.25
长句：0.5 × 0.5 × 0.5 × 0.5 = 0.0625
```

长句的总概率更小，并不表示模型对它预测得更差，只是多乘了几次 0.5。

困惑度对长度做了归一化：它先把整句的概率按词数开 $N$ 次方，再取倒数。因此它衡量的是**平均每一步预测有多不确定**，而不是整句话累积后的概率有多小。

```text
每一步平均给正确词 0.5 的概率 → PPL = 2
每一步平均给正确词 0.1 的概率 → PPL = 10
```

可以粗略地把 `PPL = 2` 理解为：模型每一步平均像是在两个同样可能的候选中选择；`PPL = 10` 则表示它平均更犹豫。**在同一个测试集上比较模型时，困惑度越低通常越好。**

### 7.2 可以用熵描述吗？可以，但实际用的是交叉熵

可以。三者的关系是：

![概率、交叉熵与困惑度的关系](assets/概率交叉熵与困惑度.svg)

| 概念 | 它在问什么 |
|---|---|
| 熵 $H$ | 真实语言分布本身有多不确定？ |
| 交叉熵 $H_{cross}$ | 模型给测试集中真实下一个词的平均惊讶程度有多大？ |
| 困惑度 $PPL$ | 把交叉熵转成“平均像有多少个候选词在犹豫”？ |

对语言模型测试集，通常计算的是交叉熵：

$$
H_{cross}=-\frac{1}{N}\sum_{t=1}^{N}\log P_{model}(w_t\mid w_{<t})
$$
...
所以困惑度不是和熵无关的新概念，而是交叉熵的另一种表达方式：交叉熵用“平均信息量 / 惊讶程度”描述，困惑度用更直观的“平均候选数量”描述。
...
从任务角度看，n-gram 和现代 LM（Language Model）做的是同一件事：
...
n-gram 是较早的统计语言模型。它直接查询“这段固定长度的前文后面，哪个词在训练集里出现得更多”。例如 trigram 只能看前两个词；再早的内容不会被使用。

后续模型没有改变“预测下一个词”这个核心目标，主要是在改进“如何记住前文”：

| 模型 | 记住前文的方式 | 主要解决的问题 |
|---|---|---|
| n-gram | 查询固定词序列的次数表 | 基础方法，但上下文短、数据稀疏 |
| RNN | 把前文逐步压缩到隐藏状态 $h$ | 可以利用比固定 n 更长的前文 |
| LSTM / GRU | 用门控制哪些信息保留、哪些忘记 | 缓解 RNN 长距离信息容易丢失的问题 |
| Transformer / 现代 LLM | 注意力机制直接参考前面的多个 token | 更好利用长上下文，也适合大规模并行训练 |

可以用同一个例子对比：

```text
前文：I ate eggs for breakfast, and later I ate scrambled ...

n-gram：只看最后 n 个词，例如 “ate scrambled”
RNN / LSTM：把整段前文压缩进一个隐藏状态
Transformer：当前 token 能直接注意到前面多个相关词，例如两次出现的 “ate” 和 “eggs”
```

因此，n-gram 的价值不只是“旧模型”。它把语言模型最核心的逻辑直接展开了：训练数据如何变成条件概率、条件概率如何变成下一个词的选择。后面的神经网络模型，本质上是在解决 n-gram 的两个限制：**只能看固定短上下文**，以及**没有见过的词序列难以泛化**。
...
- [[../自动拼写纠错/自动拼写纠错|自动拼写纠错]]：第一个接触到的 unigram 词频概率模型；
