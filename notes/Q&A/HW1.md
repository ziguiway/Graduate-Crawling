...
预测值和真实值之差，平方再平均。为什么平方？一是消除正负，二是放大偏差——差 2 变 4，差 5 变 25，**大误差被狠狠惩罚**。缺点是单位也跟着平方了。

### RMSE — 均方根误差

$$ \text{RMSE} = \sqrt{\text{MSE}} $$

把 MSE 开根号，**单位变回和原始数据一样**。RMSE = 1.1 就是"平均偏了 1.1 个单位"。

### R² — 决定系数

$$ R^2 = 1 - \frac{\text{MSE}_{model}}{\text{MSE}_{baseline}}, \quad \text{MSE}_{baseline} = \text{Var}(y_{true}) $$

跟"永远猜平均值"比。如果你一直猜 16.43，误差就是数据本身的方差（58.03）。你的模型 MSE = 1.20，只有这个的 2%。**R² 不看数据范围，最公平。**
