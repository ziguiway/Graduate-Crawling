...
### 从 0 到 Kaggle 0.98 的心路

HW1 是一个回归任务，用 93 个特征预测 COVID-19 确诊数。从啥都不会到跑通整个流程，踩坑才是真正学会的东西：
...
**最大的教训：数据预处理比调模型重要得多。**
- 测试集没做归一化 → loss 1.2，提交 score 156（直接废了）
- 归一化用了测试集自己的 mean/std → 又废一次
- 这两个坑让我彻底理解了"测试集不能看到训练集信息"是啥意思，光看书是体会不到的

**第二个教训：细节决定成败。**
- pandas 读 CSV 后 `data[1:]` 多跳了一行 → 提交行数不对
- Dataset 里少了冒号 `:` → 诡异报错查半天
- 这些语法细节，写的时候觉得"这有啥"，debug 的时候才真正记住

**最意外的：本地 dev loss 和 Kaggle 分数趋势一致。**
- 本地 R² = 0.979，Kaggle Private = 0.983
- 说明本地验证集划分是靠谱的，没有出现"本地好提交炸"的情况

### 你的最终成绩

| 指标 | 值 |
|------|-----|
| Kaggle Private Score | **0.98259**（R² ≈ 0.983）|

对比 baseline（永远猜均值）的 RMSE = 7.62，你的 RMSE = 1.10，误差缩小到 1/7。

### 现在回头看，有什么不同的做法？

1. **做好数据预处理再碰模型** — 先检查归一化、数据泄露、特征分布
2. **先用全部 93 个特征试试** — 不要一开始就拍脑袋选特征子集
3. **用 `evaluate_model` 一键评估** — 不用每次都手动看 loss 数字
4. **遇到诡异报错先怀疑"数据问题"** — 大部分 bug 不是模型结构错了，是数据形状/类型不对
