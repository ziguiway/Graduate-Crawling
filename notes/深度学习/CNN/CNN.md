...
- 感受野**彼此可以重叠**
-![350](assets/CNN/file-20260913160701255.png)
...
### 2.3 核大小（Kernel Size）——感受野的高和宽

**是什么**：描述一个感受野要用的尺寸参数，取值就是感受野的**高 × 宽**（如 $3\times3$）。
![346](assets/CNN/file-20260913161409903.png)
**为什么只讲高和宽、不讲深度？** 图像识别里通常假设"模式不会只出现在某一个通道"→ 感受野**看全部通道** → 深度自动等于通道数（RGB 图就是 3），**不用单独描述**。所以描述一个感受野，只讲高和宽就够了，合称**核大小**。

**经典取值：$3\times3$**

- 意味着我们假设"重要模式在 $3\times3$ 这么小的范围内就能被检测出来"
- $7\times7$、$9\times9$ 已经算蛮大的
- 那些大过 $3\times3$ 的模式怎么办？→ **不怕，见第 5.3 节：网络叠得越深，看得越广**

### 2.4 步幅（Stride）——感受野移动的距离

**是什么**：把感受野往右（或往下）移动的量。移动一次，就制造出一个新的守备范围（新的感受野）。
![269](assets/CNN/file-20260913161453901.png)
**性质**：是**超参数**，需要人为调整。

**经典取值：1 或 2**

- 一般不设太大——因为希望相邻感受野**有重叠**

> ❓ **Q：为什么希望感受野之间有重叠？**
> A：如果感受野完全不重叠，<font color="#ff0000">一个模式正好出现在两个感受野的**交界**上时</font>，没有任何神经元完整看到它，模式就丢了。所以感受野彼此之间要**高度重叠**，保证每个位置都被照看到。

### 2.5 填充（Padding）——感受野超出图像范围怎么办

**是什么**：当感受野有一部分超出图像范围之外，给超出部分**补值**。
![301](assets/CNN/file-20260913161541400.png)
**为什么需要**：如果不把感受野"摆"到超出图像的位置，图像**边界**的地方就没有神经元检测，出现在边界的模式就漏掉了。所以边界也要照顾到。

**补什么值**：

- 最常用**零填充（zero padding）**：超出范围当 0
- 也有别的方法，比如补整张图像所有值的平均值

### 2.6 扫描顺序：水平 + 垂直

- 水平方向按步幅扫完一整行后，**垂直方向也移动**（垂直步幅通常也是 2）
- 重复直到扫过整张图像
![396](assets/CNN/file-20260913161613883.png)
**结果**：整张图像**每一寸土地都有感受野覆盖**——图像里每个位置都有一群神经元在检测"有没有出现某些模式"。这就是简化 1 的完整落地。
...
![404](assets/CNN/file-20260913161745866.png)
...
![401](assets/CNN/file-20260913161854790.png)
...
![455](assets/CNN/file-20260913162301165.png)
...
前面讲的"神经元 + 共享参数"是**版本一**；同一件事换个视角就是**版本二：滤波器**——两者描述的是一模一样的东西。这个版本读代码时更直接（`nn.Conv2d` 里的就是它）。

#### ① 滤波器（Filter）是什么
![359](assets/CNN/file-20260913162850725.png)
- 卷积层里有一排**滤波器**（比如 64 个），每个滤波器是一个 $3 \times 3 \times \text{通道}$ 的张量（彩色图 3 通道、黑白图 1 通道）
- 滤波器里的数值就是**模型的参数**——初始未知，**通过学习找出来**
- 作用：检测一种"在 $3\times3\times$通道 范围内就能看出来"的小模式
...
```text
放 → 滤波器盖在图像左上角
算 → 滤波器的 9 个值 vs 覆盖范围的 9 个值，对应相乘再相加（内积）→ 得到一个数
移 → 按步幅右移一格，重复"算"；扫完一行再下移，直到扫完全图
```

#### ③ 一个完整例子（黑白 6×6 图像，stride=1）

设滤波器 1 检测"对角线都是 1"、滤波器 2 检测"中间一列都是 1"：
![347](assets/CNN/file-20260913191214941.png)
![394](assets/CNN/file-20260913191248098.png)
**怎么读输出**：图上哪块 $3\times3$ 区域跟滤波器**越像，内积值越大**。

- 滤波器 1 的输出里，左上角和左下角的数值最大 → 这两处出现了"对角线都是 1"的模式
- 滤波器 2 同理，它的输出指出了"竖线"出现在哪里
...
| 步骤          | 结果                                      |
| ----------- | --------------------------------------- |
| 一个滤波器扫完全图   | 得到**一组**数字（本例 $4\times4$）→ 这组数字就是一个特征映射 |
| 64 个滤波器     | 得到 **64 组**数字                           |
| 把 64 组数字摞起来 | **一张 64 通道的"新图像"**（每个通道对应一个滤波器）         |

所以整个卷积层做的事可以一句话说清：

$$
\text{图像（3 通道）} \xrightarrow{\text{过一个卷积层（64 个滤波器）}} \text{新图像（64 通道）}
$$

通道的含义也变了：输入的通道是 RGB 颜色，输出的通道是**"哪种模式被检测到了"**。
...
### 7.1 做法与性质
![395](assets/CNN/file-20260913191438050.png)
...
```
![364](assets/CNN/file-20260913191539140.png)
