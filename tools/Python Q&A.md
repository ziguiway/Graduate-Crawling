...
**一句话：Loss 告诉模型"错在哪"，Optimizer 告诉模型"怎么改"。**

```
Loss:      计算误差 → 量化"有多错"
Optimizer: 根据误差 → 更新权重，让下次误差更小
```
...
| 角色 | 作用 | 类比 |
|---|---|---|
| Loss | 计算误差 + 计算梯度 | 考试评分 + 指出错题 |
| Optimizer | 根据梯度更新参数 | 根据错题调整学习方法 |
...
PyTorch 默认会**累积梯度**，不清零的话，梯度会叠加：

```
第1个 batch: 梯度 = 0.1
第2个 batch: 梯度 = 0.1 + 0.2 = 0.3  ← 错误！应该只用第2个 batch 的梯度
```
...
|---|---|---|
| SGD | 最简单，稳定但慢 | 简单任务 |
| SGD + Momentum | 加动量，更快收敛 | 常用 |
| Adam | 自适应学习率，快且稳 | **最常用** |
| AdamW | Adam + L2 正则改进 | Transformer、大模型 |
...
x1 = 1  (one-hot)  →  w1*1 的贡献是 |w1|
x2 = 43.4 (调查数据) → w2*43.4 的贡献是 43*|w2|
```

**x2 一个特征顶 x1 四十三个特征！** 数值大的特征会支配整个输出，其他特征被忽略。
...
**梯度下降的直观理解：**
```
❌ 不归一化：又陡又长的峡谷，梯度忽大忽小，走得很慢
✅ 归一化：圆润的碗状，梯度稳定，大步往前走
```

**一句话总结：不归一化 = 让 40 米巨人跟 1 米小孩拔河，巨人肯定赢！归一化 = 把所有人都变成差不多高，公平竞争！**



### 归一化（Z-Score 标准化）的计算公式
...
```python
# 原始数据：spent_time 列
[43.4, 43.2, 43.4, 42.9, 43.3, ...]

# 第一步：计算均值 μ = 43.24
# 第二步：计算标准差 σ = 0.17
# 第三步：归一化每个数
第一个数：(43.4 - 43.24) / 0.17 = 0.94
第二个数：(43.2 - 43.24) / 0.17 = -0.24
...
```
...
| 原始值 | 归一化后 | 含义 |
|---|---|---|
| 43.4 | 0.94 | 比均值大 0.94 个标准差 |
| 43.2 | -0.24 | 比均值小 0.24 个标准差 |
| 42.9 | -2.0 | 比均值小 2 个标准差 |
...
| 方法 | 公式 | 适用场景 |
|---|---|---|
| Z-Score | `(x-μ)/σ` | 最常用，深度学习默认 |
| Min-Max | `(x-min)/(max-min)` | 缩放到 [0,1] |
| Max Abs | `x/max(|x|)` | 缩放到 [-1,1] |
