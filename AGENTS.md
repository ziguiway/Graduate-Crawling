...
- **作业问答**：李宏毅课程的作业问答命名为 `Q&AHW<编号>.md`，放在对应主题目录下（而非 `notes/Q&A/`）。其他课程或资料不要套用 `HW` 编号，使用主题名命名，例如 `Q&A_最小编辑距离.md`。`notes/Q&A/` 仅存放跨主题或尚未归类的通用问答。
- **附件**：主题目录下建 `assets/` 子目录存放该主题笔记引用的图片（遵循 obsidian-custom-attachment-location 插件约定）。
- 当用户提问学习相关问题时，先判断属于哪个主题：若主题目录已存在，把问答追加到该目录下；李宏毅作业使用 `Q&AHW*.md`，其他课程按主题命名；若主题不存在且问题足够独立，再新建主题目录。
- 笔记格式参考现有文件，使用清晰的标题和代码示例。
- 站内链接使用 `[[Wiki Link]]` 而非纯路径。
- **笔记结尾必写「我的理解」**：每篇主题主笔记末尾都要有一个"我的理解 / 个人思考"小节，记录自己消化后的感悟、疑问、与后续章节的关联等，而不是只抄书本内容。
- **QA 每节固定结构**：问答笔记里每条问答都按「**问题描述 → 回答 → 一句话记忆**」三段写，方便复习时先看问题描述自测、再看回答验证理解、最后用一句话记忆收尾。李宏毅作业问答使用 `Q&AHW*.md`，其他课程使用主题名文件。具体要求：
